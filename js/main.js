...
buyBleedButton.addEventListener('click', () => { if (!player.profession.includes("劍士")) return; const cost = getDiscountedCost(baseItemValues.bleedUpgrade.cost); if (currentMoney >= cost) { currentMoney -= cost; player.totalMoneySpent += cost; player.bleedChance += baseItemValues.bleedUpgrade.amountChance; player.bleedDamagePercentage += baseItemValues.bleedUpgrade.amountDamage; gameMessage.textContent = `成功購買！流血機率提升 ${baseItemValues.bleedUpgrade.amountChance}%，傷害提升 ${baseItemValues.bleedUpgrade.amountDamage}%！`; } else { gameMessage.textContent = '金錢不足！'; } updateDisplay(); });
